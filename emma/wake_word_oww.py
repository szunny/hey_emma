...
class OpenWakeWordDetector(BaseWakeWordDetector):
    ...
    Uses a custom ONNX model for wake-word detection.
    ...
    def __init__(self):
        ...
        model_path = os.getenv("OPENWAKEWORD_MODEL_PATH", "hey_emma.onnx")
        self.threshold = float(os.getenv("OPENWAKEWORD_THRESHOLD", "0.5"))
        self.keyword = os.getenv("OPENWAKEWORD_KEYWORD", "Hey-Emma")

        # Resolve relative paths against resource dir (like Porcupine backend)
        if model_path and not os.path.isabs(model_path):
            model_path = str(get_resource_dir() / model_path)
